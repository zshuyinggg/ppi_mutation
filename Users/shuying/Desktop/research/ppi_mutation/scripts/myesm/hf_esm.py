...
if __name__ == '__main__':
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename='hf_esm_-{epoch:02d}-{val_loss:.2f}'
    )
    ...
    trainer.fit(model=hf_esm,train_dataloaders=train_short_dataloader,val_dataloaders=val_short_dataloader)

    print('=========Start to train medium sequences============\n------------------------')
    trainer.fit(model=hf_esm,train_dataloaders=train_medium_dataloader,val_dataloaders=val_medium_dataloader)
#TODO: edit the training loop to train them all together in each epoch

    trainer=pl.Trainer(max_epochs=10, accelerator="gpu",default_root_dir=logging_path, callbacks=[early_stop_callback])
    print('=========Start to train long sequences============\n------------------------')
    trainer.fit(model=hf_esm,train_dataloaders=train_long_dataloader,val_dataloaders=val_long_dataloader)
