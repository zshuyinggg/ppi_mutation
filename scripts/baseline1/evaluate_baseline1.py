...
if __name__ == '__main__':
    ...
    if not config.get('test'):
        gcn=GNN(**config['gnn_init']).load_from_checkpoint(config['ckpt'],**config['gnn_init'])
        eval_model=evaluate_graph_context(pretrained_gnn=gcn, **config['eval_init'])
    else:
        gcn=GNN(**config['gnn_init'])
        eval_model=evaluate_graph_context(pretrained_gnn=gcn, **config['eval_init']).load_from_checkpoint(config['ckpt'])
    ...
    if not config.get('test'):trainer.fit(eval_model,datamodule=variantPPI) 
    else:trainer.test(eval_model,datamodule=variantPPI,ckpt_path=config['ckpt'])
