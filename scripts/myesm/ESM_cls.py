...
if __name__ == '__main__':
    ...
    logger=TensorBoardLogger(os.path.join(logging_path,'b0_cls'),name="%s"%args.config_file)

    checkpoint_callback = ModelCheckpoint(save_top_k=1,monitor="val_loss",mode="min",dirpath=os.path.join(logging_path,'b0_cls'),
    ...
    if not config.get('test'):trainer.fit(myesm,datamodule=proData) 
    else:trainer.test(myesm,datamodule=test_data,ckpt_path=config['ckpt'])
