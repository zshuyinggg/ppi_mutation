...
class Esm_finetune(pl.LightningModule):
    def __init__(self, esm_model=esm.pretrained.esm2_t36_3B_UR50D(), esm_model_dim=2560, truncation_len=None,
                 unfreeze_n_layers=10, repr_layers=36, lr=4 * 1e-3, crop_len=512):
        super().__init__()
        self.val_out = []
        self.save_hyperparameters()
        self.esm_model_dim = esm_model_dim
        if isinstance(esm_model, str):
            self.esm_model, alphabet = eval(esm_model)
        else:
    ...
    def random_crop_batch(self, batch, batch_idx):
        ...
        for i in range(len(batch['Name'])):
            ...
            if len(seq) > self.crop_len:
                ...
                if pos >= len(seq):
                    start = len(seq) - self.crop_len
                else:
                    start = np.random.randint(low=min_start, high=max_start + 1)
...
class Esm_cls_token(plClassificationBaseModel):
    def __init__(self, esm_model, in_dim_clf, dropout, repr_layers, lr, crop_len, seed, embedding_to_clf=['variant'],**args):
        ...
        self.embedding_to_clf = embedding_to_clf
        self.seed=seed

    def training_step(self, batch, batch_idx):
        torch.cuda.empty_cache()
        starts, labels, _, given_batch_samples, corresponding_wild_batch_samples = self.get_cropped_batch_samples(batch, batch_idx)
        given_embs = self.get_esm_embedings(given_batch_samples, starts)
        corresponding_wild_embs = self.get_esm_embedings(corresponding_wild_batch_samples, starts)
        if 'wild' in self.embedding_to_clf:
            embs = torch.hstack([given_embs, corresponding_wild_embs])
        else:
            embs = given_embs
    ...
    def get_wild_batch(self, mutated_batch, starts=None, pos=None):
        ...
        for i, seq in enumerate(seqs):
            if starts[i] == 0:  # no random cropping
                ...
                batch_sample.append(
                    (locs[i], seq[:mutant_lens[i]]))  # the wild seq has to be of same length as the mutant
            ...
            elif starts[i] is None and len(
                    seq) > self.crop_len:  # if mutated seq is not cropped but the wild is too long
    ...
    def center_crop(self, seq, pos):
        ...
        actual_left = 0
        if actual_left > ideal_left:
            left, right = actual_left, actual_left + self.crop_len
        else:
            if actual_right < ideal_right:
                left, right = actual_right - self.crop_len, actual_right
            else:
                left, right = ideal_left, ideal_right
    ...
    def crop_batch(self, batch):
        ...
        for i in range(len(batch['Name'])):
            name = names[i]
            seq = seqs[i]
            try:pos = self.get_pos_of_name(name) - 1  # it counts from 1 in biology instead 0 in python
            except TypeError: print(name), print(repr(self.get_pos_of_name(name)))
            if len(seq) > self.crop_len:
                seq_after, start = self.center_crop(seq, pos)
            else:
                seq_after = seq
...
class Esm_Transformer(plClassificationBaseModel):
    def __init__(self, esm_model=esm.pretrained.esm2_t36_3B_UR50D(), dropout=0.1, nhead=4, esm_model_dim=2560,
                 repr_layers=36, lr=4 * 1e-3, crop_len=512):
        super().__init__()
        self.save_hyperparameters()
        self.esm_model_dim = esm_model_dim
        if isinstance(esm_model, str):
        ...
        self.batch_converter = alphabet.get_batch_converter()
        self.alphabet = alphabet
        self.transformer = nn.Transformer(d_model=esm_model_dim, nhead=4, num_encoder_layers=2, num_decoder_layers=2,
                                          dim_feedforward=esm_model_dim * nhead, activation='tanh',
                                          layer_norm_eps=1e-05, batch_first=True, bias=True, dropout=dropout)
...
class Esm_finetune_delta(Esm_finetune):
    def __init__(self, esm_model=esm.pretrained.esm2_t12_35M_UR50D(), crop_val=True, esm_model_dim=480, n_class=2,
                 truncation_len=None, unfreeze_n_layers=3, repr_layers=12, batch_sample='random', which_embds='01',
                 lr=None, crop_len=512, debug=False, crop_mode='center', balanced_loss=False, local_range=0):
        """
            ...
            if detail and mem > 10:
                ...
                for name, tensor in self.named_buffers():
                    if tensor.grad is not None:
                        ...
                        if tensor_grad_memory > 0.3: print(f"{name}.grad memory usage: {tensor_grad_memory:.5f} GB",
                                                           flush=True)

                for name, param in self.named_parameters():
                    if param.grad is not None:
                        param_grad_memory = param.grad.element_size() * param.grad.numel() / 1024 ** 3  # Memory usage for the parameter gradient
                        if param_grad_memory > 0.3: print(f"{name}.grad memory usage: {param_grad_memory:.5f} GB",
                                                          flush=True)
    ...
    def training_step(self, batch, batch_idx):
        ...
        mutated_embs = self.get_esm_embedings(mutated_batch_samples, loc=locs, starts=starts,
    ...
    def define_embds(self, wild_embs, mutated_embs, verbose=False):
        ...
        l = []
        doc = ''
        wild_mean_embs, wild_local_embs, wild_AA_embs = wild_embs['sequence_representations'], wild_embs[
            'local_representations'], wild_embs['AA_representations']
        mutated_mean_embs, mutated_local_embs, mutated_AA_embs = mutated_embs['sequence_representations'], mutated_embs[
            'local_representations'], mutated_embs['AA_representations']
    ...
    def validation_step(self, batch, batch_idx):
        ...
        try:
            ...
            wild_embs = self.get_esm_embedings(wild_batch_samples, loc=locs, starts=starts,
                                               local_range=self.local_range)
            self.print_memory('wild_embs obtained', detail=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        try:
            mutated_embs = self.get_esm_embedings(mutated_batch_samples, starts=starts, loc=locs,
                                                  local_range=self.local_range)
            self.print_memory('mutated_embs obtained', detail=True)

            wild_embs = self.get_esm_embedings(wild_batch_samples, starts=starts, loc=locs,
                                               local_range=self.local_range)
            self.print_memory('wild_embs obtained', detail=True)
        except AttributeError:
            print('\n', seqs, '\n', mutated_batch_samples, '\n', wild_batch_samples, '\n')
    ...
    def get_esm_embedings(self, batch_sample, loc=None, local_range=0, starts=None):
        ...
        for i, tokens_len in enumerate(batch_lens):
            ...
            local_representations.append(token_representations[i,
                                         max(1, mutation_locs[i] - local_range + 1):min(batch_lens[i], mutation_locs[
                                             i] + local_range + 1)].mean(0))
...
class Esm_delta_multiscale_weight(Esm_finetune_delta):
    ...
    def get_esm_embedings(self, batch_sample, starts=None):
        ...
        for i in range(len(batch_lens)):
            ...
            for j, (_, one_side_len) in enumerate(self.multiscale_bins()):
                ...
                weight_right_distance = mutation_locs[i] + one_side_len + 2 - batch_lens[i] if right == batch_lens[
                    i] else 0
    ...
    def training_step(self, batch, batch_idx):
        torch.cuda.empty_cache()
        starts, labels, locs, mutated_batch_samples, wild_batch_samples = self.get_cropped_batch_samples(batch,
                                                                                                         batch_idx)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.save_embeddings:
            for i in range(embs.shape[0]):
                self.val_embds[batch['Name'][i]] = {'embs': embs[i, :].cpu(), 'UniProt': batch['UniProt'][i],
                                                    'Loc': locs[i], 'label': labels[i]}
    ...
    def on_validation_epoch_end(self):
        ...
        if self.val_embds:
            torch.save(self.val_embds,
                       '/scratch/user/zshuying/ppi_mutation/data/baseline0/val_embds_%s.pt' % self.trainer.global_rank)
    ...
    def define_embds(self, wild_embs, mutated_embs):
        ...
        mutated_mean_embs, mutated_bin_embs = mutated_embs['sequence_representations'], mutated_embs[
            'bin_representations']
        delta_mean_embs = mutated_mean_embs - wild_mean_embs
...
class Esm_multiscale_wildonly(Esm_delta_multiscale_weight):
    def __init__(self, esm_model, esm_model_dim, repr_layers, lr, unfreeze_n_layers, num_bins=8,
                 bin_one_side_distance=[0, 2, 4, 8, 16, 32, 128, 256], save_embeddings=True):
        super().__init__(esm_model, esm_model_dim, repr_layers, lr, unfreeze_n_layers, num_bins, bin_one_side_distance,
                         True)
    ...
    def define_embds(self, wild_embs):
        wild_mean_embs, wild_bin_embs = wild_embs['sequence_representations'], wild_embs['bin_representations']
        delta_mean_embs, delta_bin_embs = torch.zeros_like(wild_mean_embs).to(wild_mean_embs), torch.zeros_like(
            wild_bin_embs.to(wild_mean_embs))
        l = [wild_mean_embs, wild_bin_embs, delta_mean_embs, delta_bin_embs]
        # for item in l:
        #     print(item.device)
        return torch.hstack(l)
    ...
    def on_validation_epoch_end(self):
        torch.save(self.val_embds,
                   '/scratch/user/zshuying/ppi_mutation/data/baseline0/all_wild_embeddings_%s.pt' % self.trainer.global_rank)
...
class ESM_finetune_relative(Esm_finetune_delta):
    def __init__(self, num_heads=4, ffn_embed_dim=1280,
                 esm_model=esm.pretrained.esm2_t12_35M_UR50D(), crop_val=False, esm_model_dim=480, n_class=2,
                 truncation_len=None, unfreeze_n_layers=3, repr_layers=12, batch_sample='random', which_embds=0,
                 lr=None, crop_len=None, debug=False, crop_mode='random', balanced_loss=False, local_range=0):
        super().__init__(esm_model=esm_model, crop_val=crop_val, esm_model_dim=esm_model_dim, n_class=n_class,
                         truncation_len=truncation_len, unfreeze_n_layers=unfreeze_n_layers, repr_layers=repr_layers,
                         batch_sample=batch_sample, which_embds=which_embds, lr=lr, crop_len=crop_len,
                         crop_mode=crop_mode, balanced_loss=balanced_loss, local_range=local_range)
