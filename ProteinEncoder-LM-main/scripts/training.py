...
class BackwardRunner(ForwardRunner):
    ...
    def __init__(self,
        ...
        if lr_scheduler == 'constant':
            ...
            self.scheduler = CosineAnnealingWarmRestarts(self.optimizer, T_0=num_train_optimization_steps // 10, eta_min=1e-8, verbose=False)
...
def run_train_epoch(epoch_id: int,
    ...
    if pytorch_profiler and epoch_id == start_epoch+1:
        ...
        with torch.profiler.profile(
            ...
            #schedule=torch.profiler.schedule(skip_first=0, wait=1, warmup=1, active=2, repeat=1),
            on_trace_ready=torch.profiler.tensorboard_trace_handler(log_dir),
            record_shapes=False,
            profile_memory=True,
            with_stack=False) as pfer:
            # set timer
            start_t = timer()
...
def run_eval_epoch(eval_loader: DataLoader,
    ...
    for name in metric_names:
        ...
        elif name in ['save_embedding','save_embedding_internal']:
    ...
    for batch in tqdm(eval_loader, desc='Evaluation', total=len(eval_loader),
        ...
        if task in ['penalize_nonContact_attention','promote_contact_attention','contact_ce_attention_weightnor','contact_ce_attention','esm_eval']:
            ...
            mutants = batch['mutant'] # tuple (bs,n_mut), mutation name e.g. 'M1W'
        ...
        elif task in ['embed_seq']:
            seqId_list = batch['seq_id'] # tuple, [bs,], for save embedding
            input_mask = batch['input_mask'].cpu().numpy()
    ...
    for name, value in metric_values.items():
        if name == 'perplexity':
            ...
            metric_fun(value,set_nms[0],from_pretrained,pretrained_epoch=pretrained_epoch,save_raw_score=True)
        elif name == 'multitask_unsupervise_mutagenesis':
            pretrain_model = re.split('/',from_pretrained)[-1]
            metric_fun = registry.get_metric('multitask_unsupervise_mutagenesis')
            metric_fun(value,set_nms[0],eval_save_dir,pretrain_model,task,pretrained_epoch=pretrained_epoch,save_raw_score=True)
        elif name == 'fitness_unsupervise_scanning':
        ...
        elif name == 'save_embedding_internal':
            pretrain_model = re.split('/',from_pretrained)[-1]
...
def run_train(model_type: str,
    ...
    train_dataset = utils.setup_dataset(task, data_dir, train_split, tokenizer, data_format, in_memory=False, mlm_mask_stragy=mlm_mask_stragy, neighbor_strategy=neighbor_strategy, knn_value=knn_value, dist_cutoff=dist_cutoff)
    valid_dataset = utils.setup_dataset(task, data_dir, valid_split, tokenizer, data_format, in_memory=False, mlm_mask_stragy=mlm_mask_stragy, neighbor_strategy=neighbor_strategy, knn_value=knn_value, dist_cutoff=dist_cutoff)
    ...
    runner = BackwardRunner(
        ...
        fp16, fp16_opt_level,local_rank, max_grad_norm, warmup_steps, num_train_optimization_steps, lr_scheduler)
...
def run_eval(model_type: str,
             ...
             neighbor_strategy: str = 'knn',
             knn_value: int = 20,
             dist_cutoff: float = 12.0) -> typing.Dict[str, float]:
    ...
    valid_dataset = utils.setup_dataset(task, data_dir, split, tokenizer,
        data_format, mutgsis_set=mutgsis_set, mlm_mask_stragy=mlm_mask_stragy, neighbor_strategy=neighbor_strategy, knn_value=knn_value, dist_cutoff=dist_cutoff)
    valid_loader = utils.setup_loader(
        valid_dataset, batch_size, local_rank, n_gpu,
        1, num_workers)
